# http://emcien.github.io/number-helpers-coffeescript/
class NumberHelpers
  ...
  number_to_percentage: (float, opts={}) ->
    ...
    unless isNaN(float)
      float = @number_with_precision(float,
        precision:                  _precision
        significant:                _significant
        delimiter:                  _delimiter
        separator:                  _separator
  ...
  roundToMultiple: (number, multiple = 0.25) ->
    multipleValue = 1 / multiple
    return (Math.floor(number * multipleValue) / multipleValue)

HeraclesAdmin.helpers.number = new NumberHelpers()
