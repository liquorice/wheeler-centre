class CacheBusterController < ActionController::Metal
  ...
  def debug_response_js(page_check)
    ";(function() { if (typeof console !== 'undefined' && console !== null) { if (typeof console.debug === 'function') { console.debug('Page cache check registered #{page_check.edge_url} (#{page_check.requires_check?} – #{Time.now}) at #{page_check.updated_at}'); } } })();"
  end
end
