...
(function() {
  ...
  var InsertableEventSeriesPagesDisplayMixin = _.extend({}, InsertableDisplayMixin, {
  ...
  var InsertableEventSeriesPagesDisplay = React.createClass({

    mixins: [InsertableEventSeriesPagesDisplayMixin],
    ...
    formatPages: function() {
      if (this.state.value.pages && this.state.value.pages.length > 0) {
        ...
        return (
          <p className="insertable-display-pages__empty">No event series selected</p>
        );
      }
    }
  ...
  var InsertableEventSeriesPagesEditMixin = _.extend({}, InsertableEditMixin, {
    getInitialState: function() {
      var value = _.extend({}, {pages: [], page_ids: []}, this.props.value);
      return {
        value: value
  ...
  var InsertableEventSeriesPagesEdit = React.createClass({

    mixins: [InsertableEventSeriesPagesEditMixin],

    render: function() {
      return (
        <div className="insertable-edit fields--reversed">
          <form onSubmit={this.onSubmit}>
            <h2 className="insertable-edit__title">Edit insertable event series</h2>
            <div className="field">
              <div className="field-header">
                <label className="field-label" htmlFor="edit__caption">Event series</label>
              </div>
              <div className="field-main">
                <PagesSelector pageTypeLabel="series" pageType="event_series" page_ids={this.state.value.page_ids} callback={this.onPagesSelectorUpdate}/>
              </div>
            </div>
            <div className="field">
              <div className="field-header">
            ...
            <button type="submit" className="button button--highlight">Save changes to event series insertable</button>
          </form>
        </div>
      );
    },
  ...
  HeraclesAdmin.availableInsertables.add({
    type:    "event_series_pages",
    label:   "Event series",
    icon:    "calendar-o",
    display: InsertableEventSeriesPagesDisplay,
    edit:    InsertableEventSeriesPagesEdit
  });
