...
(function() {
  ...
  var InsertablePodcastSeriesPagesDisplayMixin = _.extend({}, InsertableDisplayMixin, {
  ...
  var InsertablePodcastSeriesPagesDisplay = React.createClass({

    mixins: [InsertablePodcastSeriesPagesDisplayMixin],
    ...
    formatPages: function() {
      if (this.state.value.pages && this.state.value.pages.length > 0) {
        ...
        return (
          <p className="insertable-display-pages__empty">No podcast series selected</p>
        );
      }
    }
  ...
  var InsertablePodcastSeriesPagesEditMixin = _.extend({}, InsertableEditMixin, {
    getInitialState: function() {
      var value = _.extend({}, {pages: [], page_ids: []}, this.props.value);
      return {
        value: value
  ...
  var InsertablePodcastSeriesPagesEdit = React.createClass({

    mixins: [InsertablePodcastSeriesPagesEditMixin],

    render: function() {
      return (
        <div className="insertable-edit fields--reversed">
          <form onSubmit={this.onSubmit}>
            <h2 className="insertable-edit__title">Edit insertable podcast series</h2>
            <div className="field">
              <div className="field-header">
                <label className="field-label" htmlFor="edit__caption">Podcast series</label>
              </div>
              <div className="field-main">
                <PagesSelector pageTypeLabel="series" pageType="podcast_series" page_ids={this.state.value.page_ids} callback={this.onPagesSelectorUpdate}/>
              </div>
            </div>
            <div className="field">
              <div className="field-header">
            ...
            <button type="submit" className="button button--highlight">Save changes to podcast series insertable</button>
          </form>
        </div>
      );
    },
  ...
  HeraclesAdmin.availableInsertables.add({
    type:    "podcast_series_pages",
    label:   "Podcast series",
    icon:    "microphone",
    display: InsertablePodcastSeriesPagesDisplay,
    edit:    InsertablePodcastSeriesPagesEdit
  });
