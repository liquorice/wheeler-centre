  ...
  task setup_broadside: :environment do
    ...
    events.each do |event_hash|
      ...
      if event.new_record?
        ...
        event.fields[:start_date].value = event_hash[:start_date] if event_hash[:start_date]
        event.fields[:start_date].time_zone = "Melbourne"
        event.fields[:end_date].value = event_hash[:end_date] if event_hash[:end_date]
        event.fields[:end_date].time_zone = "Melbourne"
        event.fields[:series].page_ids = [broadsite_event_series.id]
        event.fields[:broadside_type].value = event_hash[:type] if event_hash[:type]
        event.fields[:ticket_prices].value = event_hash[:price] if event_hash[:price]
