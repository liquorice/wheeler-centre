...
// A simple true/false checker to see if a user is new in a certain context
...
Tourist.prototype.isRegistered = function(name) {
  return !!cookie(this.cookiePrefix + name);
};

// Set a cookie for a `name` with a `stay` in milliseconds
Tourist.prototype.register = function(name, stay) {
  stay = stay || this.defaultStay;
  cookie(this.cookiePrefix + name, true, {maxage: stay});
};
